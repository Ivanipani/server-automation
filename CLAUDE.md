...
VM disks live on the **node-local** `vms` LVM-thin storage (same storage ID on every node, each backed by that node's own VG). There is **no shared storage, no live-migration, no HA**: storage being node-local, every node needs its own local template. `04-prepare-templates.yml` bakes the golden template **once** on the first baremetal node, then copies it to the others (clone locally + offline `qm/pct migrate --with-local-disks`, since `qm clone --target` needs shared storage) so all nodes hold byte-identical templates. Each node's template VMID comes from the explicit `template_vm_ids` / `template_ct_ids` maps in `inventory.yaml` (single source of truth, also read by `tofu/locals.tf` — no offset arithmetic). Each VM is created on and pinned to its `proxmox_node` and cloned from that node's local template. The bpg/proxmox v0.106 `initialization` block has no `hostname` field — VM `name` propagates to cloud-init instead. DNS + search domain reach guests via DHCP options set on OPNsense (single source of truth for DNS).
