...
- **Layout** (`k8s/clusters/doghouse/` is the Flux entrypoint): `flux-system/` (gotk-components + gotk-sync pointing at THIS repo, sync path `./k8s/clusters/doghouse`); `infra.yaml` (the infra Kustomizations); `apps.yaml` (the apps Kustomization + a dormant `doghouse-apps` GitRepository). Platform manifests are under `k8s/infra/{networking,storage,monitoring,virtualization,…}/{controllers,configs}`. **Flux `spec.path` is repo-root-relative**, so every Kustomization path is prefixed `./k8s/…` — keep that prefix on any new one.
...
- **KubeVirt guests** (`k8s/infra/virtualization/`): `controllers/` holds the vendored virt-operator + cdi-operator, `configs/` the KubeVirt + CDI CRs, and `vms/<name>/` one directory per VM (its own `virtual-machines` Flux Kustomization, `wait: false`). A guest is a **fleet host**, not just a workload: its root disk is the SAME bootserv01-published Debian 13 qcow2 the Proxmox templates clone (so the `ansible` user + baseline are already baked in), and its NIC is a Multus **bridge**-CNI attachment onto `br0` — a real MAC, a real dnsmasq reservation, reachable from the control node. It therefore also has an `ansible/inventory.yaml` entry under `virtual-machines.children.kubevirt_guests` (no `vm:` block, so Tofu never sees it); the manifest's MAC/IP and the inventory's are a **lockstep contract**. `br0` itself is Ansible's (`roles/net-bridge`, `17-host/35-network-bridge.yml`, driven by `network_bridge:` in inventory) and must exist BEFORE a VM lands — the bridge CNI silently creates an uplink-less bridge otherwise. macvlan/ipvlan are NOT options here: KubeVirt's bridge binding moves the pod interface's MAC to the VM, which those CNIs reject.
