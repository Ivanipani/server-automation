...
4. `just do-bootserv-config` — re-render bootserv01's iPXE + preseed templates (fast iteration during the netboot trial; runs the same playbook 16-foundation does)
...
- `foundation` → top-level group containing the hypervisor(s) that must be all-the-way-up (full stack + bootserv01 LXC + bootserv role) before any other baremetal can come into being. Targeted by the 16-foundation tier via `target_hosts: foundation`. Today: just `pve-home-01`. Members MUST also appear under `hypervisors`.
...
`playbooks/poochella/infra/30-guests/10-opentofu.yml` runs the **un-targeted** apply:
...
`playbooks/poochella/infra/16-foundation/80-tofu-infra-lxcs.yml` runs the **targeted** apply that brings up only the foundation infra LXCs (bootserv01 today). Same shape as `30-guests/10-opentofu.yml`, but with `targets: [module.infra_lxcs_pve_home_01]` on the `community.general.terraform` call — so it creates bootserv01 without incurring the cost (or risk) of materializing all VMs declared in the inventory. Targeting is per-invocation, not state-level — same state file as the full apply, so the later un-targeted apply reconciles everything (the LXC ends up as a no-op).
...
**Brought up by the 16-foundation tier.** The bootserv01 LXC is created by `16-foundation/80-tofu-infra-lxcs.yml` (targeted `tofu apply -target=module.infra_lxcs_pve_home_01` — does NOT touch any VM) and configured by `16-foundation/90-bootserv.yml` (was `30-guests/15-bootserv.yml`). Foundation must complete before any other baremetal can be netbooted — that's the whole point of the tier.

**Tofu ordering invariant (still relevant for the un-targeted apply in 30-guests)**: LXCs whose `lxc.tags` contains `infra` come up before any VMs. `tofu/main.tf` per-node `vms_<node>` modules declare `depends_on = [module.infra_lxcs_<node>]`, so even when the un-targeted apply runs first on a fresh fleet (no foundation tier run beforehand), bootserv01 still comes up before kube/dev VMs.
